...
- **`utilities/`** — Server support modules:
  ...
  - `post_processor.py` — Signal-adaptive post-processing (compression, EQ, normalization) applied to generated audio.
  - `tdr_nova_config.py` — TDR Nova dynamic processor configuration.
  - `generate_speaker_baselines.py` — Generate baseline audio quality scores for speaker samples.
...
### Scripts & Tests
- **`scripts/build_speaker_samples.py`** — Speaker sample curation pipeline (score, validate, concatenate voice clips).
- **`tests/`** — Test suite: `test_vocalization.py`, `test_post_processor.py`, `test_speaker_samples.py`, `audio_quality/` (full eval suite with baselines).
...
python skyrimnet_api.py   # Start FastAPI server
python SkyrimNet-LuxTTS.py  # Start Gradio server
```
